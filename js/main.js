...
function initThemeToggle() {
  ...
  /* [MÓVIL] Dos botones que FIJAN el tema (no alternan): uno pone modo claro,
     otro modo oscuro. Marcan cuál está activo con aria-pressed. */
  const setters = Array.from(document.querySelectorAll('[data-set-theme]'));
  const markActive = () => {
    const now = current();
    setters.forEach(b => b.setAttribute('aria-pressed', b.getAttribute('data-set-theme') === now ? 'true' : 'false'));
  };
  setters.forEach(btn => btn.addEventListener('click', () => {
